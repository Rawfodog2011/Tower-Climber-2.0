...
import { 
  Flame, Briefcase, Activity, Trophy, Settings, 
...
interface TutorialStep {
  ...
  icon: LucideIcon;
  color: string;
  visual?: React.ReactNode | ((t: (key: string) => string) => React.ReactNode);
}
...
const TUTORIALS_DATABASE: Record<string, TutorialData> = {
  initial: {
    ...
    subtitle: 'Protocolo de Integração: O Ciclo da Escalada',
    steps: [
      {
        title: 'Bem-vindo à Escalada',
        description: 'Você acaba de se conectar à infraestrutura da Torre de Babel. Seu objetivo é hackear e combater seu caminho através dos andares analógicos e virtuais. Para sobreviver, você precisará dominar o ciclo de Preparação, Incursão, Extração e Reinvestimento.',
        icon: Terminal,
        ...
        visual: (
          <div className="flex flex-col items-center justify-center p-4 rounded bg-slate-900/60 border border-cyan-500/20 font-mono text-xs text-cyan-300 space-y-1.5 w-full">
            ...
            <span>▶ LOOP_PROTOCOL: INITIATED</span>
            <span className="text-emerald-400">▶ OPERATOR_LEVEL: 01 (READY)</span>
          </div>
        )
      },
      {
        title: 'Fase 1: Incursão e Extração',
        description: 'Na aba "Expedição", você inicia sua incursão na Torre. Lute contra sentinelas, colete sucata e equipamentos. Mas cuidado: se o seu HP (Integridade) chegar a zero, você sofre um curto-circuito e perde parte do que coletou. Fuja (Extração) quando estiver fraco para garantir seus espólios!',
        icon: Flame,
        color: 'text-orange-400 border-orange-500/30',
        visual: (t: (key: string) => string) => (
          <div className="flex items-center justify-center gap-3 p-4 rounded bg-slate-900/60 border border-orange-500/20 w-full">
            ...
            <div className="text-left font-mono">
              <div className="text-xs font-bold text-orange-300 uppercase tracking-widest">{t("EXTRAÇÃO BEM SUCEDIDA")}</div>
              <div className="text-[10px] text-emerald-400">{t("+ 450 XP | + 120 Ouro")}</div>
              <div className="text-[10px] text-slate-400">{t("Loot mantido no inventário.")}</div>
            </div>
          </div>
        )
      },
      {
        title: 'Fase 2: Reinvestimento',
        description: 'Use o Ouro, a Sucata e a Experiência obtidos nas incursões para evoluir! No Hub, você usará a "Forja" para criar armamentos melhores, e a "Matriz Neural" (Habilidades) para desbloquear novos ataques e bônus passivos.',
        icon: Zap,
        color: 'text-yellow-400 border-yellow-500/30',
        visual: (t: (key: string) => string) => (
          <div className="grid grid-cols-2 gap-2 p-3 rounded bg-slate-900/60 border border-yellow-500/20 font-mono text-[10px] w-full text-yellow-300">
            <div className="flex justify-between border-b border-yellow-950/40 pb-1"><span>{t("Forja:")}</span> <span>{t("Criar Armas")}</span></div>
            <div className="flex justify-between border-b border-yellow-950/40 pb-1"><span>{t("Soldagem:")}</span> <span>{t("Fundir Chips")}</span></div>
            <div className="flex justify-between pb-1"><span>{t("Matriz:")}</span> <span>{t("Evoluir Classe")}</span></div>
            <div className="flex justify-between pb-1"><span>{t("Adaptações:")}</span> <span>{t("Cibernética")}</span></div>
          </div>
        )
      },
      {
        title: 'Fase 3: Preparação',
        description: 'Antes de voltar à Torre, visite a aba "Geral" para equipar o loot coletado e os itens forjados. Oculte o capacete, otimize suas braçadeiras e calças. Quanto melhor seu equipamento, mais alto você subirá na Escalada.',
        icon: Briefcase,
        ...
        visual: (t: (key: string) => string) => (
          <div className="flex items-center gap-4 p-3 rounded bg-slate-900/60 border border-purple-500/20 w-full text-left font-mono text-[10px]">
            ...
            <div>
              <div className="text-purple-300 font-bold">{t("Equipamento Otimizado")}</div>
              <div className="text-slate-400 text-[9px]">{t("Dano Físico +45 | Defesa +30")}</div>
              <div className="text-emerald-400 text-[9px]">{t("Pronto para incursão.")}</div>
            </div>
          </div>
        )
      }
          ...
          <div className="px-6 py-4 border-t border-slate-900 bg-slate-950/80 flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              {currentStep < stepsCount - 1 && (
                <button
                  onClick={onComplete}
                  className="px-3 py-2 text-slate-500 hover:text-slate-300 font-mono text-xs uppercase tracking-wider transition-colors cursor-pointer"
                >
                  {t("Pular")}
                </button>
              )}
