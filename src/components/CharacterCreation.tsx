...
import { Shield, Zap, Sparkles, Target, Cpu, User, ArrowRight, Terminal, Bot, Ghost, UserRound, Crosshair, Fingerprint, Eye, Hexagon, Dice5 } from 'lucide-react';
...
export function CharacterCreation() {
  ...
  const handleConfirmOrigin = () => {
    onComplete(selectedId, playerName.trim() || undefined, selectedAvatar);
  };

  const handleRandomName = () => {
    const random = CYBERPUNK_NAMES[Math.floor(Math.random() * CYBERPUNK_NAMES.length)];
  ...
  return (
    <div className="fixed inset-0 bg-slate-950 text-slate-100 font-sans z-50 flex flex-col overflow-y-auto">
      ...
      <div className="max-w-6xl w-full mx-auto px-4 py-8 md:py-12 flex flex-col flex-grow justify-center relative z-10">
        ...
        {step === 1 ? (
          <div className="w-full max-w-2xl mx-auto flex flex-col gap-6 animate-in fade-in zoom-in-95 duration-500">
            <div className="text-center mb-6">
              <div className="inline-flex items-center gap-2 bg-cyan-950/50 border border-cyan-500/30 px-3 py-1.5 rounded-full text-xs text-cyan-400 font-mono tracking-widest uppercase mb-4 animate-pulse">
                <Terminal className="w-3.5 h-3.5" /> {t("PROTOCOLO DE INICIALIZAÇÃO")}
              </div>
              <h1 className="text-3xl md:text-5xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 via-blue-400 to-indigo-500 tracking-tight">
                {t("Identidade do Operador")}
              </h1>
            </div>

            <div className="system-panel bg-slate-900/30 border border-slate-800 rounded-xl p-6 md:p-8 flex flex-col gap-8 relative overflow-hidden">
              <div className="absolute top-0 right-0 w-4 h-4 border-t-2 border-r-2 border-cyan-500/30 rounded-tr"></div>
              <div className="absolute bottom-0 left-0 w-4 h-4 border-b-2 border-l-2 border-cyan-500/30 rounded-bl"></div>
              ...
              <div className="flex flex-col gap-2">
                ...
                <div className="flex gap-2">
                  <input 
                    ...
                    placeholder={t("Digite um nome ou gere um aleatório...")}
                    className="flex-grow bg-slate-950 border border-slate-700 rounded px-4 py-3 text-slate-100 font-mono focus:outline-none focus:border-cyan-500 transition-colors"
                  />
                  <button 
                    onClick={handleRandomName}
                    className="bg-slate-800 hover:bg-slate-700 border border-slate-600 px-4 rounded flex items-center justify-center transition-colors"
                    title={t("Gerar Nome Aleatório")}
                  >
              ...
              <div className="flex flex-col gap-2 pt-4 border-t border-slate-800">
                <label className="text-xs font-mono font-bold text-purple-400 uppercase tracking-widest">
                  {t("Módulo de Síntese Vocal")}
                </label>
                <SystemVoiceSelector />
              </div>
              ...
              <div className="mt-4 pt-4 border-t border-slate-800/60">
                <button
                  onClick={() => setStep(2)}
                  className="w-full font-bold px-6 py-4 rounded-lg flex items-center justify-center gap-2 transition-all uppercase tracking-wider cursor-pointer bg-cyan-600 hover:bg-cyan-500 text-slate-950 shadow-[0_0_20px_rgba(6,182,212,0.25)] hover:shadow-[0_0_30px_rgba(6,182,212,0.4)]"
                >
                  {t("Confirmar Identidade")} <ArrowRight className="w-5 h-5 text-slate-950" />
                </button>
              </div>
            </div>
          </div>
        ) : (
          <div className="animate-in fade-in slide-in-from-right-8 duration-500 flex flex-col flex-grow">
            {/* Top Header */}
            <div className="text-center mb-8 md:mb-12 relative">
              <button 
                onClick={() => setStep(1)}
                className="absolute left-0 top-0 md:top-2 text-xs font-mono text-slate-400 hover:text-cyan-400 flex items-center gap-1 transition-colors"
              >
                &larr; {t("Voltar")}
              </button>
              <div className="inline-flex items-center gap-2 bg-cyan-950/50 border border-cyan-500/30 px-3 py-1.5 rounded-full text-xs text-cyan-400 font-mono tracking-widest uppercase mb-4 animate-pulse">
                      ...
                      <div>
                        ...
                        <div className="mb-6">
                          <div className="flex items-center justify-between mb-2">
                            ...
                            <TTSButton
                              text={t(selectedOrigin.lore)}
                              id={`origin-lore-${selectedOrigin.id}`}
                              variant={isSelectedSecret ? 'red' : 'cyan'}
                              size="sm"
                            />
                          </div>
